...
st.caption(f"Python: {sys.version.split()[0]} • Build: 2025-11-10-progress+exam+primary")
...
def compute_topic_progress(topic_folder_id: str) -> float:
    ...
    try:
        ...
        if quiz_ids:
            ...
            # take average of latest attempt per quiz
...
if "folder" in params and "sb_user" in st.session_state:
    ...
    try:
        ...
        if subs:
            ...
            for s in subs:
                # Show progress if this is a Topic level (has items under it)
                progress = compute_topic_progress(s["id"])
                cols = st.columns([5,2,1])
                cols[0].button(f"📁 {s['name']}", key=f"open_{s['id']}", on_click=lambda fid=s['id']: (_set_params(folder=fid), st.rerun()))
                cols[1].progress(progress, text=f"{int(progress*100)}%")
                if cols[2].button("Open", key=f"open_btn_{s['id']}"): _set_params(folder=s["id"]); st.rerun()
    except Exception:
        pass
...
# ---------- Home tabs (Exam Planner, Quick Study, Manage Items) ----------
tabs = st.tabs(["Exam Planner", "Quick Study", "Manage Items"])

# Exam Planner (Subject → Exam → Topic; upload & generate)
with tabs[0]:
    ...
    if "sb_user" not in st.session_state:
        ...
        subjects = [r for r in roots]
        subj_names = [s["name"] for s in subjects]
        subj = st.selectbox("Subject", ["(create new)"] + subj_names)
        new_subject = st.text_input("New subject", placeholder="e.g., A-Level Mathematics")

        subject_id = None
        if subj != "(create new)" and subj_names:
            subject_id = next(s["id"] for s in subjects if s["name"] == subj)
        elif st.button("Create subject"):
        ...
        if subject_id:
            ...
            ex = st.selectbox("Exam", ["(create new)"] + exam_names)
            new_exam = st.text_input("New exam", placeholder="e.g., May 2026")

            exam_id = None
            if ex != "(create new)" and exam_names:
                exam_id = next(e["id"] for e in exams if e["name"]==ex)
            elif st.button("Create exam"):
            ...
            if exam_id:
                ...
                tp = st.selectbox("Topic", ["(create new)"] + topic_names)
                new_topic = st.text_input("New topic", placeholder="e.g., Differentiation")

                topic_id = None
                if tp != "(create new)" and topic_names:
                    topic_id = next(t["id"] for t in topics if t["name"]==tp)
                elif st.button("Create topic"):
                ...
                if topic_id:
                    ...
                    files = st.file_uploader("Upload files (PDF, PPTX, JPG, PNG, TXT) for this topic",
                                             type=["pdf","pptx","jpg","jpeg","png","txt"], accept_multiple_files=True)

                    if files and st.button("Generate Notes + Flashcards + Quiz", type="primary"):
...
with tabs[1]:
    ...
    if "sb_user" not in st.session_state:
        ...
        audience_label = st.selectbox("Audience", ["University", "A-Level", "A-Level / IB", "GCSE", "HKDSE", "Primary"], index=0)
        aud_map = {
            "University": "university",
            "A-Level": "A-Level",
            "A-Level / IB": "A-Level",
        ...
        audience = aud_map.get(audience_label, "high school")
        detail = st.slider("Detail level", 1, 5, 3)
        subject_hint = st.text_input("Subject (e.g., Mathematics)", value="General")
