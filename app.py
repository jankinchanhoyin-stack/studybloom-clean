...
st.caption(f"Python: {sys.version.split()[0]} • Build: 2025-11-10-flip+grade+folders-FIX")
...
def interactive_flashcards(flashcards: List[dict], key_prefix: str = "fc"):
...
def interactive_quiz(questions: List[dict], item_id: Optional[str] = None, key_prefix: str = "quiz", subject_hint: str = "General"):
    st.subheader("🧪 Quiz")
    if not questions:
    ...
    st.session_state.setdefault(f"{key_prefix}_history", [])        # per Q: {"score": int, "max": int}
    ...
    if c3.button("✅ Finish & Save", key=f"{key_prefix}_finish"):
        if item_id and "sb_user" in st.session_state:
            try:
                # convert to simple correct/total using >=70% rule per question
                correct = sum(
                    1 for h in st.session_state[f"{key_prefix}_history"]
                    if h.get("max", 0) and h.get("score", 0) >= 0.7 * h["max"]
                )
                total = len(questions)
    ...
    # Generate another quiz from sibling notes
            ...
            if folder_id:
                ...
                if summary and summary.get("data"):
                    ...
                    if new_qs:
                        ...
                        _set_params(item=created.get("id"))
                        st.rerun()
                    else:
...
# ====================== Folder helpers (TOP-LEVEL, not nested!) ======================
...
# ====================== Sidebar: ONLY subject (root) folders ======================
...
if "sb_user" in st.session_state:
    ...
    try:
        ...
        roots, _ = build_tree(all_folders)  # show only roots
        for n in roots:
        ...
        with st.sidebar.expander("New Subject"):
            ...
            if st.button("Create", use_container_width=True, key="create_root"):
                if not (new_name or "").strip():
                    ...
                    try:
                        created = create_folder(new_name.strip(), None)
                        _set_params(folder=created["id"])
                        st.rerun()
                    except Exception as e:
...
# ====================== Home tabs (Quick Study, Manage Items) ======================
        ...
        if files and st.button("Generate & Save", type="primary"):
            ...
            try:
                ...
                if data.get("flashcards"):
                    flash_id = save_item(
                        "flashcards", f"{emoji['flashcards']} {title} • Flashcards",
                        {"flashcards": data["flashcards"]}, dest_id
                    ).get("id")
                if data.get("exam_questions"):
                    quiz_id = save_item(
                        "quiz", f"{emoji['quiz']} {title} • Quiz",
                        {"questions": data["exam_questions"]}, dest_id
                    ).get("id")
                st.success("Saved ✅")
            except Exception as e:
            ...
            if summary_id and c1.button("Open Notes Page", type="primary"):
                _set_params(item=summary_id)
                st.rerun()
            if flash_id and c2.button("Open Flashcards Page"):
                _set_params(item=flash_id)
                st.rerun()
            if quiz_id and c3.button("Open Quiz Page"):
...
with tabs[1]:
    ...
    if "sb_user" not in st.session_state:
        ...
        filter_id = None if folder_filter == "(all folders)" else next(
            f["id"] for f in all_folders if f["name"] == folder_filter
        )
        try:
            ...
            for it in items:
                ...
                cols = st.columns([6, 1, 1])
                cols[0].markdown(f"{icon} **{it['title']}** — {it['created_at'][:16].replace('T',' ')}")
                if cols[1].button("Open", key=f"open_{it['id']}"):
                    _set_params(item=it["id"])
                    st.rerun()
                if cols[2].button("Delete", key=f"del_{it['id']}"):
                    try:
