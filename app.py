        ...
        with inner:
            ...
            if "sb_user" not in st.session_state:
                if c2.button("Log in", key="tb_login"):
                    _route_set("login")
                if c2.button("Sign up", key="tb_signup"):
                    _route_set("signup")
            else:
                if c2.button("My Account", key="tb_myacct"):
                    _route_set("account")
...
def main():
    ...
    if "item" in params and "sb_user" in st.session_state:
        page_item()
    elif st.session_state.get("route") == "login":
        page_login()
    elif st.session_state.get("route") == "signup":
        page_signup()
    elif st.session_state.get("route") == "account":
        page_account()
    else:
        page_home()
